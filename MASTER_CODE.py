...
nc_file = 'C:/Users/KVasilakou/OneDrive - Universiteit Antwerpen/PhD/GIS/Eutrophication CFs/GLOBAL/FUTURE/Discharge/pcr-globwb_miroc5_ewembi_rcp60_2005soc_co2_dis_global_daily_2091_2099.nc4'
...
for year in range(2091, 2100):
    ...
    yearly_avg.to_netcdf(resampled_nc_file) #m3/s
...
# Load the NetCDF file
nc_file = 'C:/Users/KVasilakou/OneDrive - Universiteit Antwerpen/PhD/GIS/Eutrophication CFs/GLOBAL/FUTURE/Runoff/pcr-globwb_hadgem2-es_ewembi_rcp60_2005soc_co2_qtot_global_daily_2061_2070.nc4'
data = xr.open_dataset(nc_file)
...
nc_file = 'C:/Users/KVasilakou/OneDrive - Universiteit Antwerpen/PhD/GIS/Eutrophication CFs/GLOBAL/FUTURE/Irrigation water use/pcr-globwb_miroc5_ewembi_rcp60_2005soc_co2_airrww_global_monthly_2006_2099.nc4'
...
# Pad the area array to match the shape (360, 720)
padded_area = np.pad(area, ((0, 60), (0, 0)), mode='constant', constant_values=np.nan)

# Calculate the start and end indices for each year from 2021 to 2099
start_index = (2021 - 2006) * 12  # Starting month index for year 2021
end_index = (2099 - 2006 + 1) * 12  # Ending month index for year 2099 (inclusive)

# Define the number of seconds in each month (considering leap years is omitted for simplicity)
days_in_month = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
seconds_in_month = np.array(days_in_month) * 24 * 60 * 60

# Loop through each year from 2021 to 2099
for year in range(2021, 2100):
    # Calculate the indices for the current year
    start_month = (year - 2006) * 12  # Starting month index for the current year
    end_month = start_month + 12  # Ending month index for the current year

    # Select data for the current year
    data_year = data.isel(time=slice(start_month, end_month))

    # Calculate the total kg/m2 for each month by multiplying by the number of seconds in each month
    #monthly_totals = data_year * seconds_in_month[:, np.newaxis, np.newaxis]

    # Sum these monthly totals to get the yearly total
    ...
    # Output file name
    output_file = f'Irr_0.5_{year}.nc'

    # Save the yearly total data to a new NetCDF file
    yearly_total_m3.to_netcdf(output_file)

    print(f'Yearly total data for {year} saved to {output_file}')
